...
def seed_metro_schedules(cur):
    ...
    for s in data:
        ...
        for idx, sid in enumerate(s["stops_in_order"]):
            stops_rows.append((
                s["schedule_id"],
                sid,
                idx + 1,  # 1-indexed stop_order
                s["travel_time_from_origin_min"][sid]
            ))
    insert_many(
        cur,
        "metro_schedules",
    ...
    print(f"  metro_schedules seeded: {len(data)} schedules, {len(stops_rows)} stops seeded.")
...
def seed_national_rail_schedules(cur):
    ...
    print(f"  national_rail_schedules seeded: {len(data)} schedules, {len(stops_rows)} stops, {len(fares_rows)} class fares seeded.")
